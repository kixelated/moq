...
export default function WatchControlsContextProvider(
    props: WatchControlsContextProviderProps
) {
    ...
    createEffect(() => {
        ...
        if (!hangWatchEl.active) {
            ...
            hangWatchEl.addEventListener(
                'watch-instance-available',
                (event: CustomEvent) => {
                    const watchInstance =
                        event.detail.instance.peek?.() as HangWatchInstance;
                    onWatchInstanceAvailable(watchInstance);
                }
            );
        } else {
            const hangWatchInstance =
                hangWatchEl.active.peek?.() as HangWatchInstance;
            onWatchInstanceAvailable(hangWatchInstance);
        }
    });

    return (
        <WatchControlsContext.Provider value={value}>
            {props.children}
        </WatchControlsContext.Provider>
    );
    ...
    function onWatchInstanceAvailable(watchInstance: HangWatchInstance) {
        ...
        watchInstance?.signals.effect(function trackBuffering(effect) {
            const syncStatus = effect.get(
                watchInstance?.video.source.syncStatus
            );
            const bufferStatus = effect.get(
                watchInstance?.video.source.bufferStatus
            );
            const shouldShow =
                syncStatus.state === 'wait' || bufferStatus.state === 'empty';
