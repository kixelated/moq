...
import type { SourceType } from '@kixelated/hang/publish/element';
...
export const PublishControlsContext =
    createContext<PublishControlsContextValue>();

export default function PublishControlsContextProvider(
    props: PublishControlsContextProviderProps
) {
    ...
    const [microphoneDevices, setMicrophoneMediaDevices] = createSignal<
        MediaDeviceInfo[]
    >([]);
    const [selectedMicrophoneSource, setSelectedMicrophoneSource] =
        createSignal<MediaDeviceInfo['deviceId'] | undefined>();
    ...
    const [publishStatus, setPublishStatus] =
        createSignal<PublishStatus>('no-url');
    ...
    createEffect(() => {
        const el = props.hangPublish();
        if (!el) return; // ref not set yet, do nothing

        if (!el.active) {
            // @ts-ignore ignore custom event
            el.addEventListener(
                'publish-instance-available',
                (event: CustomEvent) => {
                    const publishInstance =
                        event.detail.instance.peek?.() as HangPublishInstance;

                    onPublishInstanceAvailable(el, publishInstance);
                }
            );
        } else {
            const publishInstance = el.active.peek?.() as HangPublishInstance;
            onPublishInstanceAvailable(el, publishInstance);
        }
    });

    return (
        <PublishControlsContext.Provider value={value}>
            {props.children}
        </PublishControlsContext.Provider>
    );

    function onPublishInstanceAvailable(
        el: HangPublish,
        publishInstance: HangPublishInstance
    ) {
        publishInstance?.signals.effect((effect) => {
        ...
        publishInstance?.signals.effect((effect) => {
            ...
            setSelectedCameraSource(video.device.requested.peek());
        });
        ...
        publishInstance?.signals.effect((effect) => {
            ...
            setSelectedMicrophoneSource(audio.device.requested.peek());
        });
