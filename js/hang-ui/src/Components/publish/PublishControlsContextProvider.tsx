...
export default function PublishControlsContextProvider(
    ...
    createEffect(() => {
        ...
        if (!hangPublishEl.active) {
            ...
            const publishInstance =
                hangPublishEl.active.peek?.() as HangPublishInstance;
            onPublishInstanceAvailable(hangPublishEl, publishInstance);
        }
    });
    ...
    function onPublishInstanceAvailable(
        ...
        publishInstance?.signals.effect(
            function trackMicrophoneDevices(effect) {
                const audio = effect.get(publishInstance.audio);

                if (!audio || !('device' in audio)) return;

                const enabled = effect.get(
                    publishInstance.broadcast.audio.enabled
                );
                if (!enabled) return;
        ...
        publishInstance?.signals.effect(
            function trackNothingSourceActive(effect) {
        ...
        publishInstance?.signals.effect(
            function trackMicrophoneSourceActive(effect) {
        ...
        publishInstance?.signals.effect(
            function trackVideoSourcesActive(effect) {
        ...
        publishInstance?.signals.effect(
            function trackSelectedCameraSource(effect) {
                const video = effect.get(publishInstance.video);
        ...
        publishInstance?.signals.effect(
            function trackSelectedMicrophoneSource(effect) {
                const audio = effect.get(publishInstance.audio);
