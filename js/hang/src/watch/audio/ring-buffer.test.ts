...
test("initialization", async (t) => {
	await t.test("should initialize with valid parameters", () => {
		...
		buffer.initialize({ sampleRate: 48000, channelCount: 2, latency: 100 });
	...
	await t.test("should throw on invalid channel count", () => {
		...
		assert.throws(
			() => buffer.initialize({ sampleRate: 48000, channelCount: 0, latency: 100 }),
			/invalid channels/,
		);
	});
	...
	await t.test("should throw on invalid sample rate", () => {
		...
		assert.throws(() => buffer.initialize({ sampleRate: 0, channelCount: 2, latency: 100 }), /invalid sample rate/);
	});

	await t.test("should throw on invalid latency", () => {
		const buffer = new AudioRingBuffer();

		assert.throws(() => buffer.initialize({ sampleRate: 48000, channelCount: 2, latency: 0 }), /invalid latency/);
	});
...
test("writing data", async (t) => {
	await t.test("should write continuous data", () => {
		const buffer = new AudioRingBuffer();
		buffer.initialize({ sampleRate: 1000, channelCount: 2, latency: 100 }); // 100 samples buffer

		// Write 10 samples at timestamp 0
		buffer.write(0, createData(10, 2, 1.0));
		assert.strictEqual(buffer.length, 10);

		// Write 10 more samples at timestamp 0.01 (10ms)
		buffer.write(0.01, createData(10, 2, 2.0));
	...
	await t.test("should handle gaps by filling with zeros", () => {
		...
		buffer.write(0.02, createData(10, 2, 2.0));
	...
	await t.test("should handle late-arriving samples", () => {
		const buffer = new AudioRingBuffer();
		buffer.initialize({ sampleRate: 1000, channelCount: 1, latency: 100 }); // 100 samples buffer, 1 channel for simplicity

		// Fill buffer to exit refill mode
		buffer.write(0, createData(100, 1, 0.0));
		buffer.write(0.1, createData(10, 1, 0.0)); // This exits refill mode
		...
		buffer.write(0.12, createData(10, 1, 1.0));

		// Now write data that fills the gap at timestamp 0.11 (110ms)
		buffer.write(0.11, createData(10, 1, 2.0));
	...
	await t.test("should discard samples that are too old", () => {
		...
		buffer.write(0, createData(100, 2, 0.0));
		buffer.write(0.1, createData(10, 2, 0.0)); // This exits refill mode
		const clearOutput = createOutput(110, 2);
		buffer.read(clearOutput);

		// Write 50 samples
		buffer.write(0.11, createData(50, 2, 1.0));
		...
		buffer.write(0.11, createData(5, 2, 2.0)); // These should be ignored
	...
	await t.test("should throw when writing to uninitialized buffer", () => {
		const buffer = new AudioRingBuffer();
		assert.throws(() => buffer.write(0, createData(10)), /not initialized/);
	});
});
...
test("reading data", async (t) => {
	await t.test("should read available data", () => {
		const buffer = new AudioRingBuffer();
		buffer.initialize({ sampleRate: 1000, channelCount: 2, latency: 100 });
		...
		// Write 20 samples
		buffer.write(0.11, createData(20, 2, 1.5));
	...
	await t.test("should handle partial reads", () => {
		...
		// Write 20 samples
		buffer.write(0.11, createData(20, 2, 1.0));
...
test("refill behavior", async (t) => {
	await t.test("should start in refill mode", () => {
		...
		buffer.write(0, createData(50, 2, 1.0));
		const output = createOutput(10, 2);
		const samplesRead = buffer.read(output);
	...
	await t.test("should exit refill mode when buffer is full", () => {
		...
		buffer.write(0, createData(100, 2, 1.0));

		// Write more data to trigger overflow handling
		buffer.write(0.1, createData(50, 2, 2.0)); // This should exit refill mode
...
test("ring buffer wrapping", async (t) => {
	await t.test("should wrap around when buffer is full", () => {
		const buffer = new AudioRingBuffer();
		buffer.initialize({ sampleRate: 1000, channelCount: 1, latency: 100 }); // 100 samples buffer, 1 channel

		// Fill the buffer
		buffer.write(0, createData(100, 1, 1.0));

		// Write more data, causing wrap
		buffer.write(0.1, createData(50, 1, 2.0));
		assert.strictEqual(buffer.refilling, false);
		...
		buffer.write(0.15, createData(50, 1, 3.0));
...
test("status reporting", async (t) => {
	await t.test("should report correct status", () => {
		...
		// Fill buffer
		buffer.write(0.05, createData(50, 2, 1.0));
		status = buffer.status();
...
test("multi-channel handling", async (t) => {
	await t.test("should handle different channel counts correctly", () => {
		const buffer = new AudioRingBuffer();
		buffer.initialize({ sampleRate: 1000, channelCount: 2, latency: 100 }); // 2 channel buffer
		...
		buffer.write(0.11, monoData);

		// Write stereo data
		const stereoData = createData(10, 2, 2.0);
		buffer.write(0.12, stereoData);
...
test("edge cases", async (t) => {
	...
	await t.test("should handle fractional timestamps", () => {
		...
		buffer.write(0.1105, createData(10, 2, 1.0)); // 110.5 samples, rounds to 111
		buffer.write(0.1204, createData(10, 2, 2.0)); // 120.4 samples, rounds to 120
